...
{
  fins::ParamLoader config("finenav_mppi_controller.PathAlignCritic");
  enabled_                  = config.get("enabled", true)
                                    .with_description("Enable PathAlignCritic");
  power_                    = config.get("cost_power", 1)
                                    .with_description("Power exponent applied to cost");
  weight_                   = config.get("cost_weight", 14.0f)
                                    .with_description("Weight for path alignment cost");
  max_path_occupancy_ratio_ = config.get("max_path_occupancy_ratio", 0.05f)
                                    .with_description("Max fraction of path points that may be occupied before disabling critic");
  offset_from_furthest_     = config.get("offset_from_furthest", 20)
                                    .with_description("Offset from furthest reachable path point for alignment target");
  trajectory_point_step_    = config.get("trajectory_point_step", 4)
                                    .with_description("Step size between trajectory points evaluated for alignment");
  threshold_to_consider_    = config.get("threshold_to_consider", 0.5f)
                                    .with_description("Distance to goal below which critic deactivates (m)");
  use_path_orientations_    = config.get("use_path_orientations", false)
                                    .with_description("Include path point orientation in alignment cost");
}
