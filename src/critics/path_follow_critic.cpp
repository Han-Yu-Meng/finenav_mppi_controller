...
{
  fins::ParamLoader config("finenav_mppi_controller.PathFollowCritic");
  enabled_               = config.get("enabled", true)
                                 .with_description("Enable PathFollowCritic");
  power_                 = config.get("cost_power", 1)
                                 .with_description("Power exponent applied to cost");
  weight_                = config.get("cost_weight", 5.0f)
                                 .with_description("Weight for path following cost");
  offset_from_furthest_  = config.get("offset_from_furthest", 6)
                                 .with_description("Offset from furthest reachable path point for follow target");
