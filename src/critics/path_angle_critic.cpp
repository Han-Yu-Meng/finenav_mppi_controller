...
{
  fins::ParamLoader config("finenav_mppi_controller.PathAngleCritic");
  enabled_               = config.get("enabled", true)
                                 .with_description("Enable PathAngleCritic");
  power_                 = config.get("cost_power", 1)
                                 .with_description("Power exponent applied to cost");
  weight_                = config.get("cost_weight", 2.0f)
                                 .with_description("Weight for path angle cost");
  offset_from_furthest_  = config.get("offset_from_furthest", 4)
  ...
  max_angle_to_furthest_ = config.get("max_angle_to_furthest", 1.0f)
