...
{
  fins::ParamLoader config("finenav_mppi_controller.PathAlignLegacyCritic");
  enabled_                  = config.get("enabled", false)
                                    .with_description("Enable PathAlignLegacyCritic (legacy variant, prefer PathAlignCritic)");
  power_                    = config.get("cost_power", 1)
                                    .with_description("Power exponent applied to cost");
  weight_                   = config.get("cost_weight", 10.0f)
                                    .with_description("Weight for path alignment cost");
  max_path_occupancy_ratio_ = config.get("max_path_occupancy_ratio", 0.07f)
                                    .with_description("Max fraction of path points that may be occupied");
  offset_from_furthest_     = config.get("offset_from_furthest", 20)
                                    .with_description("Offset from furthest reachable path point");
  trajectory_point_step_    = config.get("trajectory_point_step", 4)
                                    .with_description("Step size between trajectory points");
