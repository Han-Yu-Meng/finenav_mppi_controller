...
{
  fins::ParamLoader config("finenav_mppi_controller.TwirlingCritic");
  enabled_ = config.get("enabled", false)
                   .with_description("Enable TwirlingCritic (penalizes unnecessary spinning)");
  power_   = config.get("cost_power", 1)
                   .with_description("Power exponent applied to cost");
  weight_  = config.get("cost_weight", 10.0f)
                   .with_description("Weight for twirling cost");
}
