...
public:
   ...
   */
        ...
        arams)
    {
        ...
        // std::cout<<"[finenav_mppi_controller] Optimizing initialized"<<std::endl;
    }
    ...
    void getParams()
  {
      ...
      // ── Optimizer settings ───────────────────────────────────────────────────
      s.model_dt        = config.get("model_dt", 0.05f)
                                .with_description("Time step duration (s). controller_period = model_dt is required.");
      s.time_steps      = config.get("time_steps", 56)
                                .with_description("Number of time steps in trajectory rollout");
      s.batch_size      = config.get("batch_size", 2000)
                                .with_description("Number of sampled trajectories per iteration");
      s.iteration_count = config.get("iteration_count", 1)
                                .with_description("Number of optimization iterations per control cycle");
      s.temperature     = config.get("temperature", 0.3f)
                                .with_description("MPPI temperature: higher = more exploration, lower = more exploitation");
      s.gamma           = config.get("gamma", 0.015f)
                                .with_description("Control cost regularization coefficient");

      // ── Velocity constraints ─────────────────────────────────────────────────
      s.base_constraints.vx_max = config.get("vx_max", 0.5)
                                        .with_description("Maximum forward velocity (m/s)");
      s.base_constraints.vx_min = config.get("vx_min", -0.35)
                                        .with_description("Minimum forward velocity (m/s). Negative = allow reverse.");
      s.base_constraints.vy     = config.get("vy_max", 0.0)
                                        .with_description("Maximum lateral velocity (m/s). 0 for non-holonomic.");
      s.base_constraints.wz     = config.get("wz_max", 1.9)
                                        .with_description("Maximum angular velocity (rad/s)");

      // ── Sampling standard deviations ─────────────────────────────────────────
      s.sampling_std.vx = config.get("vx_std", 0.2)
                                .with_description("Sampling standard deviation for forward velocity");
      s.sampling_std.vy = config.get("vy_std", 0.2)
                                .with_description("Sampling standard deviation for lateral velocity");
      s.sampling_std.wz = config.get("wz_std", 0.4)
                                .with_description("Sampling standard deviation for angular velocity");
  ...
  void evalControl(
      ...
      const finenav_mppi_controller::IMapView& map_view)
    {
...
protected:
  ...
  void optimize(const finenav_mppi_controller::IMapView& map_view)
  {
   ...
   */
    geometry_msgs::msg::TwistStamped getControlFromSequenceAsTwist(
        const builtin_interfaces::msg::Time & stamp, const finenav_mppi_controller::IMapView& map_view)
    {
